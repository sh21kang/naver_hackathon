...
def bind_model(model):
    ...
    def infer(queries, db):
        ...
        #     ranked_list2 = ranked_list[:N]
        #     retrieval_results2[query] = ranked_list2
        
        # count =[[0 for j in range(len(indices[0]))] for i in range(len(queries))]
    
        # for (i,ref) in enumerate(indices): # ref
        #     for (j, ref2) in enumerate(ref[:N]):
        #         count[i][ref2] = count[i][ref2] +1

        # for (i,ref) in enumerate(indices2):
        #     for (j, ref2) in enumerate(ref[:N]):
        #         count[i][ref2] = count[i][ref2] +1
...
if __name__ == '__main__':
    ...
    nsml.load(47,session='kh_square/ir_ph2/221')
