...
def rmac_regions(W, H, L):
    ...
    print("rmac_regions(W:",W," H:",H," L:",L," )")
    ovr = 0.4 # desired overlap of neighboring regions
    steps = np.array([2, 3, 4, 5, 6, 7], dtype=np.float) # possible regions for the long dimension

    w = min(W,H)
    print("w:",w," = min(W:",W," H:",H," )")

    b = (max(H,W) - w)/(steps-1)
    print("b:",b," = (max(H:",H," ,W:",W," ) - w:",w," )/(steps:",steps," -1)")
    idx = np.argmin(abs(((w ** 2 - w*b)/w ** 2)-ovr)) # steps(idx) regions for long dimension
    print("idx = np.argmin(abs(((w ** 2 - w*b)/w ** 2)-ovr))")
    print("{0} = np.argmin(abs((({1} ** 2 - {2}*{3})/{4} ** 2)-{5}))".format(idx,w,w,b,w,ovr))
