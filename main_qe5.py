...
def bind_model(model):
    ...
    def infer(queries, db):
        ...
        for (i, query) in enumerate(queries):
            ...
            queries3_vecs.append(reference_vecs[indices[i][1]])
            queries4_vecs.append(reference_vecs[indices[i][1]])
        ...
        sim_matrix3=np.dot(queries3_vecs, reference_vecs.T)
        indices3 = np.argsort(sim_matrix3, axis=1)
        indices3 = np.flip(indices3, axis=1)

        sim_matrix4=np.dot(queries4_vecs, reference_vecs.T)
        indices4 = np.argsort(sim_matrix4, axis=1)
        indices4 = np.flip(indices4, axis=1)
        ...
        indices3_at_1000=indices3[:,:N]
        indices4_at_1000=indices4[:,:N]

        
        for (i,ref) in enumerate(indices1_at_1000): 
            for (j, ref2) in enumerate(ref[:N]):
                ...
                if ref2 in indices3_at_1000[i]:
                    new_sim_matrix[i][ref2]=new_sim_matrix[i][ref2]+weight*j*(len(query_vecs)-j)
                if ref2 in indices4_at_1000[i]:
                    new_sim_matrix[i][ref2]=new_sim_matrix[i][ref2]+weight*j*(len(query_vecs)-j)
                else:
                    pass
